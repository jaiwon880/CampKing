...
class GetResult:
    ...
    def choice_address(self) : 
        if self.choice != "" : 
            return plus_index(self.df[self.df['시, 군'] == self.choice].reset_index(drop=True))
            
        elif self.address != None :
            # return self.plus_index(self.df[self.df['글램핑장'].str.contains(self.address).reset_index(drop=True)])
            return plus_index(self.df[(self.df['시, 군'] == self.choice) & (self.df['글램핑장'].str.contains(self.address))].reset_index(drop=True))
