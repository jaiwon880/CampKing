...
class GetSideBar:
    def __init__(self) -> None:
        ...
        self.area = [""] + self.df.iloc[:, 0].unique().tolist()
        self.area_choice = self.sb.selectbox('지역 선택 그..치만....경기도 뿐인걸...', self.area)

        self.direction = [""] + self.df[self.df[self.area_choice] == self.area_choice].drop_duplicates(subset=iloc[:, 1])[iloc[:, 0]].tolist() if self.area_choice != "" else None
